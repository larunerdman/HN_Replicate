---
title: "HN NoCovariate Model 20220127"
author: "LaurenErdman"
date: "27/01/2022"
output: 
  html_document:
...
# raw_in = readLines(paste0(results_wd,"/NoFinalLayerFineTuneNoCov_TrainOnly_40epochs_bs16_lr0.001_RCFalse_covFalse_OSFalse.json"))
raw_in = readLines(paste0(results_wd,"/NoFinalLayerFineTuneNoCov_v2_TrainOnly_40epochs_bs16_lr0.001_RCFalse_covFalse_OSFalse.json"))
  ## initial and v2 model results nearly identical -- leaving it here for transparency b/c it was run a second time to get model weights

test = v8()
test$assign("dat", JS(raw_in))
st_stan_test = test$get("dat")

st_stan_test = add_platt_preds_dprep(in_list = st_stan_test, epoch = 30,sensitivity = SENSITIVITY, verbose = TRUE)
...
write.csv(val_df[,c("id","mod_pred_ep30","target01","target")], paste0(results_wd, "/Datasheets/Val_NoCov_predictions_20220213.csv"),
          quote=FALSE,row.names=FALSE)

write.csv(st_hn_only[,c("id","st_pred_mod_ep30","target01","target")], paste0(results_wd, "/Datasheets/SilentTrial_NoCov_predictions_20220213.csv"),
          quote=FALSE,row.names=FALSE)
...
(st_overall_origmod_auc = get_auroc_aurpc(score_vec = st_hn_only$st_pred_mod_ep30, 
...
nocov_list = list()
nocov_list[["st_overall"]] = list()
nocov_list[["st_overall"]][["aucs"]] = st_overall_origmod_auc
nocov_list[["st_overall"]][["sen_spec"]] = st_overall_origmod
...
nocov_list[["st_mostrec"]] = list()
nocov_list[["st_mostrec"]][["aucs"]] = st_mostrec_origmod_auc
nocov_list[["st_mostrec"]][["sen_spec"]] = st_mostrec_origmod
...
write.csv(stan_merge[,c("id","stan_pred_mod_ep30","target01","target")], paste0(results_wd, "/Datasheets/Stanford_NoCov_predictions_20220213.csv"),
          quote=FALSE,row.names=FALSE)
...
nocov_list[["stan_all"]] = list()
nocov_list[["stan_all"]][["aucs"]] = stan_all_origmod_auc
nocov_list[["stan_all"]][["sen_spec"]] = stan_all_origmod
...
write.csv(ui_merge[,c("id","ui_pred_mod_ep30","target01","target")], paste0(results_wd, "/Datasheets/UIowa_NoCov_predictions_20220213.csv"),
          quote=FALSE,row.names=FALSE)
...
(ui_all_origmod_auc = get_auroc_aurpc(score_vec = ui_df$ui_pred_mod_ep30, 
...
nocov_list[["ui_all"]] = list()
nocov_list[["ui_all"]][["aucs"]] = ui_all_origmod_auc
nocov_list[["ui_all"]][["sen_spec"]] = ui_all_origmod 
...
write.csv(chop_merge[,c("id","chop_pred_mod_ep30","target01","target")], paste0(results_wd, "/Datasheets/CHOP_NoCov_predictions_20220213.csv"),
          quote=FALSE,row.names=FALSE)
...
nocov_list[["chop_all"]] = list()
nocov_list[["chop_all"]][["aucs"]] = chop_all_origmod_auc
nocov_list[["chop_all"]][["sen_spec"]] = chop_all_origmod 

saveRDS(object = nocov_list, file = paste0(results_wd,"/nocov_model_list_results_20220207.rds"))
