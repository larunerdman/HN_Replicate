---
title: "HN Original Model Results"
author: "LaurenErdman"
date: "04/12/2021"
output: 
  html_document:
...
# Original SickKids model (Tables 1-4)
...
raw_in = readLines(paste0(results_wd,"NoFinalLayerFineTuneSickKidsTrained_TrainOnly_40epochs_bs16_lr0.001_RCFalse_covTrue_OSFalse.json"))
test = v8()
...
val_df = data.frame(mod_pred_ep30 = st_stan_test$val[[1]][[30]]$pred_scaled, 
                    id = st_stan_test$val[[1]][[30]]$id,
                      target01 = st_stan_test$val[[1]][[30]]$target)
val_df$target = factor(val_df$target01, levels = c(0,1), labels = c("no Obstruction", "Obstruction"))

# write.csv(val_df[,c("id","mod_pred_ep30","target01","target")], paste0(results_wd, "/Datasheets/Val_predictions_20220201.csv"),
#           quote=FALSE,row.names=FALSE)

# write.csv(st_hn_only[,c("id","st_pred_mod_ep30","target01","target")], paste0(results_wd, "/Datasheets/SilentTrial_predictions_20220201.csv"), 
#           quote=FALSE,row.names=FALSE)
...
## Overall 
table(st_hn_only$target)
sum(table(st_hn_only$target))

## Unique patients
table(st_hn_only$target[!duplicated(st_hn_only$ID)])
sum(table(st_hn_only$target[!duplicated(st_hn_only$ID)]))

```


#### Model performance (Tables 2 and 3)
##### Overall 
```{r}
...
(st_aucs_overall_origmod = get_auroc_aurpc(score_vec = st_hn_only$st_pred_mod_ep30, 
...
original_model_list[["st_overall_origmod"]] = list()
original_model_list[["st_overall_origmod"]][["aucs"]] = st_aucs_overall_origmod
original_model_list[["st_overall_origmod"]][["sen_spec"]] = st_overall_origmod
...
auroc_auprc_split(score_vec = st_hn_only$st_pred_mod_ep30, 
                  target = st_hn_only$target01, 
...
#### Model performance: Most recent observations only (Table 2 and STable 3)
##### Overall
```{r}
...
## overall 
(st_mostrec_origmod_auc = get_auroc_aurpc(score_vec = hn_only_most_recent$st_pred_mod_ep30, 
...
original_model_list[["st_mostrec_origmod"]] = list()
original_model_list[["st_mostrec_origmod"]][["aucs"]] = st_mostrec_origmod_auc
original_model_list[["st_mostrec_origmod"]][["sen_spec"]] = st_mostrec_origmod
...
# write.csv(stan_merge[,c("id","stan_pred_mod_ep30","target01","target")], paste0(results_wd, "/Datasheets/Stanford_predictions_20220201.csv"),
#           quote=FALSE,row.names=FALSE)
...
(stan_all_origmod_auc = get_auroc_aurpc(score_vec = stan_df$stan_pred_mod_ep30, 
...
original_model_list[["stan_all_origmod"]] = list()
original_model_list[["stan_all_origmod"]][["aucs"]] = stan_all_origmod_auc
original_model_list[["stan_all_origmod"]][["sen_spec"]] = stan_all_origmod
...
(stan_40test_origmod_auc = get_auroc_aurpc(score_vec = stan40_test$stan_pred_mod_ep30, 
...
original_model_list[["stan_40test_origmod"]] = list()
original_model_list[["stan_40test_origmod"]][["aucs"]] = stan_40test_origmod_auc
original_model_list[["stan_40test_origmod"]][["sen_spec"]] = stan_40test_origmod
...
# write.csv(ui_merge[,c("id","ui_pred_mod_ep30","target01","target")], paste0(results_wd, "/Datasheets/UIowa_predictions_20220201.csv"),
#           quote=FALSE,row.names=FALSE)
...
## 40% Test -- for comparison with fine-tuning and combined-train model performance
table(ui40_test$target)
...
(ui_all_origmod_aucs = get_auroc_aurpc(score_vec = ui_df$ui_pred_mod_ep30, 
...
original_model_list[["uiowa_all_origmod"]] = list()
original_model_list[["uiowa_all_origmod"]][["aucs"]] = ui_all_origmod_aucs
original_model_list[["uiowa_all_origmod"]][["sen_spec"]] = ui_all_origmod

```

##### 40% Test (Tables 5 and 6)
```{r}

(ui_40test_origmod_auc = get_auroc_aurpc(score_vec = ui40_test$ui_pred_mod_ep30, 
...
original_model_list[["uiowa_40test_origmod"]] = list()
original_model_list[["uiowa_40test_origmod"]][["aucs"]] = ui_40test_origmod_auc
original_model_list[["uiowa_40test_origmod"]][["sen_spec"]] = ui_40test_origmod
...
chop_merge$kidney_side = unlist(lapply(strsplit(x = chop_merge$id,split = "_"), function(x){x[2]}))
...
# write.csv(chop_merge[,c("id","chop_pred_mod_ep30","target01","target")], paste0(results_wd, "/Datasheets/CHOP_predictions_20220201.csv"),
#           quote=FALSE,row.names=FALSE)
...
## overall target
table(chop_merge$target)

## Age group
table(chop_merge$age_grp)
table(chop_merge$age_grp, chop_merge$target)

## Sex
table(chop_merge$Sex)
table(chop_merge$Sex, chop_merge$target)

## HN side
table(chop_merge$Laterality.of.hydro)
table(chop_merge$Laterality.of.hydro, chop_merge$target)

## 40% Test -- for comparison with fine-tuning and combined-train model performance
table(chop40_test$target)
...
(chop_all_origmod_auc = get_auroc_aurpc(score_vec = chop_df$chop_pred_mod_ep30, 
...
original_model_list[["chop_all_origmod"]] = list()
original_model_list[["chop_all_origmod"]][["aucs"]] = chop_all_origmod_auc
original_model_list[["chop_all_origmod"]][["sen_spec"]] = chop_all_origmod
...
(chop_40test_origmod_auc = get_auroc_aurpc(score_vec = chop40_test$chop_pred_mod_ep30, 
...
original_model_list[["chop_40test_origmod"]] = list()
original_model_list[["chop_40test_origmod"]][["aucs"]] = chop_40test_origmod_auc
original_model_list[["chop_40test_origmod"]][["sen_spec"]] = chop_40test_origmod

saveRDS(object = original_model_list, file = paste0(results_wd,"/original_model_list_results_20220207.rds"))
...
auroc_auprc_split(score_vec = chop_merge$chop_pred_mod_ep30, 
               target = chop_merge$target01, 
               split_vec = chop_merge$kidney_side, 
               n_it = 500)
...
raw_in = readLines(paste0(results_wd, "/FineTune/FinalLayerFineTuneStanfordFinetune60%_TrainOnly_40epochs_bs16_lr0.001_RCFalse_covTrue_OSFalse.json"))
test = v8()
test$assign("dat", JS(raw_in))
st_stan_test = test$get("dat")
st_stan_test = add_platt_preds_dprep(in_list = st_stan_test, epoch = 10,sensitivity = SENSITIVITY, verbose=FALSE)

st_preds = data.frame(st_pred_mod_ep30 = st_stan_test$st[[1]][[10]]$pred_scaled,
                      id = st_stan_test$st[[1]][[10]]$id,
                      target01 = st_stan_test$st[[1]][[10]]$target)
st_preds$target = factor(st_preds$target01, levels = c(0,1), labels = c("no Obstruction", "Obstruction"))
st_df = st_preds
# st_df$id

thresh_ep30 = st_stan_test[["val"]][[fold]][[10]][["threshold"]]

st_details = read.csv(paste0(results_wd, "/Datasheets/SilentTrial_Datasheet_deID_20211205.csv"),header = TRUE, as.is = TRUE)

st_merged = make_st_df(st_df=st_preds, st_details=st_details)

st_hn_only = st_merged[["overall"]]
hn_only_most_recent = st_merged[["most_recent"]]
...
## Stanford
```{r}
stan_preds = data.frame(stan_pred_mod_ep30 = st_stan_test$stan[[1]][[10]]$pred_scaled,
                        id = st_stan_test$stan[[1]][[10]]$id,
                        target = st_stan_test$stan[[1]][[10]]$target)
...
```{r}
get_auroc_aurpc(score_vec = stan_merge$stan_pred_mod_ep30, 
                    target = stan_merge$target01, 
                    n_it = 500)
...
ui_preds = data.frame(ui_pred_mod_ep30 = st_stan_test$ui[[1]][[10]]$pred_scaled,
                      id = st_stan_test$ui[[1]][[10]]$id,
                      target01 = st_stan_test$ui[[1]][[10]]$target)
ui_preds$target = factor(ui_preds$target01, levels = c(0,1), labels = c("no Obstruction", "Obstruction"))
ui_df = ui_preds
ui_df$pt_id = unlist(lapply(strsplit(as.character(ui_df$id),"_"),function(x){x[1]}))
...
chop_preds = data.frame(chop_pred_mod_ep30 = st_stan_test$chop[[1]][[10]]$pred_scaled,
                        id = st_stan_test$chop[[1]][[10]]$id,
                        target = st_stan_test$chop[[1]][[10]]$target)
...
raw_in = readLines(paste0(results_wd,"/FineTune/FinalLayerFineTuneUIowaFinetune60%_TrainOnly_40epochs_bs16_lr0.001_RCFalse_covTrue_OSFalse.json"))
test = v8()
test$assign("dat", JS(raw_in))
st_stan_test = test$get("dat")
st_stan_test = add_platt_preds_dprep(in_list = st_stan_test, epoch = 1, sensitivity = SENSITIVITY, verbose = FALSE)

st_preds = data.frame(st_pred_mod_ep1 = st_stan_test$st[[1]][[1]]$pred_scaled,
                      id = st_stan_test$st[[1]][[1]]$id,
                      target01 = st_stan_test$st[[1]][[1]]$target)
st_preds$target = factor(st_preds$target01, levels = c(0,1), labels = c("no Obstruction", "Obstruction"))

fold=1

thresh_ep1 = st_stan_test[["val"]][[fold]][[1]][["threshold"]]
...
##### All visits
```{r}
get_auroc_aurpc(score_vec = st_hn_only$st_pred_mod_ep1, 
                    target = st_hn_only$target01, 
                    n_it = 500)

(st_overall_uift = get_sen_spec_withCI(score_vec = st_hn_only$st_pred_mod_ep1, 
                    target = st_hn_only$target01, 
                    threshold = thresh_ep1, 
                    n_it = 500, out_vecs = FALSE))
...
##### Most recent
```{r}
get_auroc_aurpc(score_vec = hn_only_most_recent$st_pred_mod_ep1, 
                    target = hn_only_most_recent$target01, 
                    n_it = 500)
...
## Stanford
```{r}
stan_preds = data.frame(stan_pred_mod_ep1 = st_stan_test$stan[[1]][[1]]$pred_scaled,
                        id = st_stan_test$stan[[1]][[1]]$id,
                        target = st_stan_test$stan[[1]][[1]]$target)
stan_preds$target = factor(stan_preds$target, levels = c(0,1), labels = c("no Obstruction", "Obstruction"))
...
##### Overall
```{r}
get_auroc_aurpc(score_vec = stan_merge$stan_pred_mod_ep1, 
                    target = stan_merge$target01, 
                    n_it = 500)
...
##### 40% Test
```{r}
get_auroc_aurpc(score_vec = stan40_test$stan_pred_mod_ep1, 
                    target = stan40_test$target01, 
                    n_it = 500)

(stan_overall_uift = get_sen_spec_withCI(score_vec = stan40_test$stan_pred_mod_ep1, 
                    target = stan40_test$target01, 
                    threshold = thresh_ep1, 
                    n_it = 500))
...
## UIowa
```{r}
ui_preds = data.frame(ui_pred_mod_ep1 = st_stan_test$ui[[1]][[1]]$pred_scaled,
                        id = st_stan_test$ui[[1]][[1]]$id,
                        target01 = st_stan_test$ui[[1]][[1]]$target)
ui_preds$target = factor(ui_preds$target01, levels = c(0,1), labels = c("no Obstruction", "Obstruction"))
...
get_auroc_aurpc(score_vec = ui_df$ui_pred_mod_ep1, 
                    target = ui_df$target01, 
                    n_it = 500)
...
chop_preds = data.frame(chop_pred_mod_ep30 = st_stan_test$chop[[1]][[1]]$pred_scaled,
                        id = st_stan_test$chop[[1]][[1]]$id,
                        target = st_stan_test$chop[[1]][[1]]$target)
...
raw_in = readLines(paste0(results_wd, "/FineTune/FinalLayerFineTuneCHOPFinetune60%_TrainOnly_40epochs_bs16_lr0.001_RCFalse_covTrue_OSFalse.json"))
test = v8()
test$assign("dat", JS(raw_in))
st_stan_test = test$get("dat")
st_stan_test = add_platt_preds_dprep(in_list = st_stan_test, epoch = 10,sensitivity = SENSITIVITY, verbose=FALSE)
...
get_auroc_aurpc(score_vec = stan40_test$stan_pred_mod_ep30, 
                    target = stan40_test$target01, 
                    n_it = 500)
(stan_overall_stft = get_sen_spec_withCI(score_vec = stan40_test$stan_pred_mod_ep30, 
                    target = stan40_test$target01, 
                    threshold = thresh_ep30, 
                    n_it = 500))
```
...
get_auroc_aurpc(score_vec = ui40_test$ui_pred_mod_ep30, 
                    target = ui40_test$target01, 
                    n_it = 500)
(ui_overall_stft = get_sen_spec_withCI(score_vec = ui40_test$ui_pred_mod_ep30, 
                    target = ui40_test$target01, 
                    threshold = thresh_ep30, 
                    n_it = 500))
...
# CHOP-Stanford-UIowa-SickKids train model (Table 6)
## SickKids Silent trial: All visits
```{r, warning=FALSE}
raw_in = readLines(paste0(results_wd,"/CombinedTrain/FinalLayerFineTuneCombinedTrain60%_TrainOnly_40epochs_bs16_lr0.001_RCFalse_covTrue_OSFalse.json"))
test = v8()
test$assign("dat", JS(raw_in))
st_stan_test = test$get("dat")
st_stan_test = add_platt_preds_dprep(in_list = st_stan_test, epoch = 20,sensitivity = SENSITIVITY, verbose=FALSE)
st_preds = data.frame(st_pred_mod_ep20 = st_stan_test$st[[1]][[20]]$pred_scaled,
                     id = st_stan_test$st[[1]][[30]]$id,
...
fold=1
dat_thresh_ep20 = st_stan_test[["val"]][[fold]][[20]][["threshold"]]
st_merged = make_st_df(st_df=st_preds, st_details=st_details)
st_hn_only = st_merged[["overall"]]
hn_only_most_recent = st_merged[["most_recent"]]

get_auroc_aurpc(score_vec = st_hn_only$st_pred_mod_ep20,
                   target = st_hn_only$target01, 
                    n_it = 500)
...
## SickKids Silent trial: Most recent
```{r}

get_auroc_aurpc(score_vec = hn_only_most_recent$st_pred_mod_ep20,
                   target = hn_only_most_recent$target01, 
                    n_it = 500)
...
## Stanford data
```{r}
stan_preds = data.frame(stan_pred_mod_ep20 = st_stan_test$stan[[1]][[20]]$pred_scaled,
                       id = st_stan_test$stan[[1]][[30]]$id,
...
get_auroc_aurpc(score_vec = stan_merge$stan_pred_mod_ep20,
                   target = stan_merge$target01, 
                    n_it = 500)
...
### UIowa data
```{r}
ui_preds = data.frame(ui_pred_mod_ep20 = st_stan_test$ui[[1]][[20]]$pred_scaled,
                     id = st_stan_test$ui[[1]][[30]]$id,
                     target01 = st_stan_test$ui[[1]][[30]]$target)
ui_preds$target = factor(ui_preds$target01, levels = c(0,1), labels = c("no Obstruction", "Obstruction"))
ui_df = ui_preds
ui_df$pt_id = unlist(lapply(strsplit(as.character(ui_df$id),"_"),function(x){x[1]}))
ui_merge = merge(ui_df, ui_datasheet, by.y = "Name", by.x = "pt_id")

get_auroc_aurpc(score_vec = ui_df$ui_pred_mod_ep20,
                   target = ui_df$target01, 
                    n_it = 500)
...
chop_preds = data.frame(chop_pred_mod_ep30 = st_stan_test$chop[[1]][[20]]$pred_scaled,
                        id = st_stan_test$chop[[1]][[10]]$id,
                        target = st_stan_test$chop[[1]][[10]]$target)
...
<!-- # Embedding plots (Figure 2) -->

<!-- ```{r embeddings, echo=FALSE} -->

<!-- train_embed = read.csv(paste0(results_wd,"/Embeddings/Training_embeddings.csv"), header=FALSE, as.is=TRUE) -->
<!-- val_embed = read.csv(paste0(results_wd,"/Embeddings/Validation_embeddings.csv"), header=FALSE, as.is=TRUE) -->
<!-- sitrial_embed = read.csv(paste0(results_wd,"/Embeddings/SilentTrial_embeddings.csv"), header=FALSE, as.is=TRUE) -->
<!-- stan_embed = read.csv(paste0(results_wd,"/Embeddings/Stanford_embeddings.csv"), header=FALSE, as.is=TRUE) -->
<!-- uiowa_embed = read.csv(paste0(results_wd,"/Embeddings/UIowa_embeddings.csv"), header=FALSE, as.is=TRUE) -->

<!-- train_ids = train_embed[,ncol(train_embed)] -->
<!-- val_ids = val_embed[,ncol(val_embed)] -->
<!-- sitrial_ids = sitrial_embed[,ncol(sitrial_embed)] -->
<!-- stan_ids = stan_embed[,ncol(stan_embed)] -->
<!-- uiowa_ids = uiowa_embed[,ncol(uiowa_embed)] -->

<!-- all_train_ids = c(train_ids, val_ids) -->
<!-- # length(unique(all_train_ids)) -->
<!-- # length(all_train_ids) -->

<!-- # saveRDS(unique(all_train_ids), file = "C:/Users/lauren erdman/Desktop/kidney_img/HN/SickKids/training_ids_20210803.rds") -->

<!-- all_dat = rbind(train_embed, val_embed, sitrial_embed,  -->
<!--                 stan_embed, uiowa_embed) -->

<!-- all_embed = all_dat[,-ncol(all_dat)] -->
<!-- all_embed = all_embed[,-1] -->

<!-- ids = all_dat[,ncol(all_dat)] -->
<!-- col_vec = rep("blue", length(ids)) -->
<!-- col_vec[ids %in% val_ids] = "dodgerblue" -->
<!-- col_vec[ids %in% sitrial_ids] = "deeppink2" -->
<!-- col_vec[ids %in% stan_ids] = "darkorchid2" -->
<!-- col_vec[ids %in% uiowa_ids] = "forestgreen" -->
<!-- col_vec[substr(ids,1,4) == "ORIG"] = "darkturquoise" -->

<!-- group_vec = rep("TrainingST", length(ids)) -->
<!-- group_vec[ids %in% val_ids] = "Validation" -->
<!-- group_vec[ids %in% sitrial_ids] = "SilentTrial" -->
<!-- group_vec[ids %in% stan_ids] = "Stanford" -->
<!-- group_vec[ids %in% uiowa_ids] = "UIowa" -->
<!-- group_vec[substr(ids,1,4) == "ORIG"] = "TrainingOrig" -->

<!-- # length(group_vec[group_vec == "TrainingOrig"]) -->
<!-- # length(group_vec[group_vec == "TrainingST"]) -->

<!--   ## 2  -->
<!-- group_vec2 = rep("Training", length(ids)) -->
<!-- group_vec2[ids %in% val_ids] = "Validation" -->
<!-- group_vec2[ids %in% sitrial_ids] = "SilentTrial" -->
<!-- group_vec2[ids %in% stan_ids] = "Stanford" -->
<!-- group_vec2[ids %in% uiowa_ids] = "UIowa" -->
<!-- group_vec2[substr(ids,1,4) == "ORIG"] = "Training" -->


<!-- my_umap = umap(all_embed) -->

<!-- # head(my_umap$layout) -->


<!-- umap_df = data.frame(cbind(ids,my_umap$layout,col = col_vec, group = group_vec)) -->
<!-- names(umap_df)[2:3] = c("Dim1","Dim2") -->

<!-- gg_df = data.frame(cbind(my_umap$layout,group_vec2)) -->
<!-- names(gg_df)[1:2] = c("Dim1","Dim2") -->
<!-- gg_df$Dim1 = as.numeric(gg_df$Dim1) -->
<!-- gg_df$Dim2 = as.numeric(gg_df$Dim2) -->

<!-- gg_df$DataSet = factor(gg_df$group_vec2, levels = c("Training","Validation","SilentTrial","Stanford","UIowa")) -->

<!-- ## GGPLOT THIS GRAPH AND FACET GRID IT -->

<!-- theme_set( -->
<!--   theme_bw(base_size = 20) -->
<!-- ) -->
<!-- ggplot(gg_df, aes(x = Dim1, y = Dim2, col = DataSet)) +  -->
<!--   geom_point(size = 2, shape = 18) +  -->
<!--   scale_color_manual(name = "Data Set", -->
<!--                      values = c("black","orange","skyblue","deeppink1","aquamarine4")) +  -->
<!--   guides(colour = guide_legend(override.aes = list(size=5)))#+ facet_grid(~group_vec) -->

<!-- ggplot(gg_df, aes(x = Dim1, y = Dim2, col = DataSet)) +  -->
<!--   geom_point(size = 2, shape = 18) +  -->
<!--   scale_color_manual(name = "Data Set", -->
<!--                      values = c("black","orange","skyblue","deeppink1","aquamarine4")) +  -->
<!--   guides(colour = guide_legend(override.aes = list(size=5)))+ facet_grid(~DataSet) -->
...
<!-- ### color by surgery status  -->

<!-- training_df = readRDS(file = paste0(results_wd, "/Datasheets/training_details.rds")) -->

<!-- # head(training_df) -->
<!-- # head(umap_df) -->

<!-- umap_train = umap_df[umap_df$ids %in% training_df$full_id,] -->
<!-- # head(umap_train) -->
<!-- # dim(umap_train) -->
<!-- #  -->
<!-- # dim(umap_df) -->

<!-- umap_merge = merge(umap_train, training_df, by.x = "ids", by.y = "full_id") -->
<!-- # head(umap_merge) -->
<!-- umap_merge$Dim1 = as.numeric(umap_merge$Dim1) -->
<!-- umap_merge$Dim2 = as.numeric(umap_merge$Dim2) -->
<!-- umap_merge$Obstruction = factor(umap_merge$surgery, levels = c(0,1), -->
<!--                                 labels = c("No","Yes")) -->
...
<!-- ggplot(umap_merge, aes(x = Dim1, y = Dim2, col = Obstruction)) +  -->
<!--   geom_point()  +  -->
<!--   geom_point(size = 2, shape = 18) +  -->
<!--   scale_color_manual(name = "Obstruction", -->
<!--                      values = c("#009E73","#CC79A7")) +  -->
<!--   ggtitle("Color by Obstruction") +  -->
<!--   guides(colour = guide_legend(override.aes = list(size=5, shape=18)))#+ facet_grid(~group_vec) -->

<!-- ggplot(umap_merge, aes(x = Dim1, y = Dim2, col = group)) +  -->
<!--   geom_point() #+ facet_grid(~group_vec) -->

<!-- ggplot(umap_merge, aes(x = Dim1, y = Dim2, col = age_grp)) +  -->
<!--   geom_point() #+ facet_grid(~group_vec) -->

<!-- ggplot(umap_merge, aes(x = Dim1, y = Dim2, col = apd_group)) +  -->
<!--   geom_point() #+ facet_grid(~group_vec) -->

<!-- ggplot(umap_merge, aes(x = Dim1, y = Dim2, col = USMachine)) +  -->
<!--   geom_point() #+ facet_grid(~group_vec) -->

<!-- ggplot(umap_merge, aes(x = Dim1, y = Dim2, col = apd_group)) +  -->
<!--   geom_point() #+ facet_grid(~group_vec) -->

<!-- ggplot(umap_merge, aes(x = Dim1, y = Dim2, col = side)) +  -->
<!--   geom_point() #+ facet_grid(~group_vec) -->

<!-- ggplot(umap_merge, aes(x = Dim1, y = Dim2, col = set)) +  -->
<!--   geom_point() +  -->
<!--   scale_color_manual(values = c("blue","dodgerblue","deeppink2","darkorchid2","forestgreen","darkturquoise")) #+ facet_grid(~group_vec) -->
